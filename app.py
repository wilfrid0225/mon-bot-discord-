    ...
    if id_a_verifier in SALONS_INTERDITS:
        ...
        if not (est_admin or a_role):
            ...
            if nb >= 3:
                ...
                # Sanction : Exclusion temporaire (timeout) de 24 heures
                await membre.timeout(datetime.timedelta(hours=24), reason="3 infractions")
                await envoyer_log(discord.Embed(description=f"{membre.name} a été exclu pour 24h après 3 infractions.", color=discord.Color.orange()))
            else:
                infractions[uid] = nb
                sauvegarder_donnees("infractions.json", infractions)
